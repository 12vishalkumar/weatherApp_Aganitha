...
async function checkWeather(city) {
  ...
  if (response.status == 404) {
    ...
    var unix = ("0" + date.getDate()).slice(-2) + "/" + ("0" + (date.getMonth() + 1)).slice(-2) + "/" + date.getFullYear();
    //------------------------------ query selector function ---------------------------//
    document.querySelector(".temp").innerHTML = Math.round(data.main.temp) + "°C";
    document.querySelector(".city").innerHTML = data.name;
    document.querySelector(".wind").innerHTML = data.wind.speed;
    document.querySelector(".humid").innerHTML = data.main.humidity;
    document.querySelector(".feeling").innerHTML = Math.round(data.main.feels_like);
...
//---------------------------------- add Event Listener function for click -------------//
searchBtn.addEventListener("click", () => {
  //-------------------------------- check Weather function call -----------------------//
